...
public class Shader {
  ...
  public void init(final GL3 gl) {
    ...
    shadersHandles = new int[sources.length];
  ...
  public void dispose() {
    unbind();
    for (int shaderHandle : this.shadersHandles) {
      if (shaderHandle < 0) { continue; }
      gl.glDetachShader(programHandle, shaderHandle);
  ...
  private int linkShaders(int[] shadersHandles) {
    final int program_handle = gl.glCreateProgram();

    for (int i = 0; i < shadersHandles.length; i++) {
      if (shadersHandles[i] >= 0) {
        gl.glAttachShader(program_handle, shadersHandles[i]);
      }
    }

    gl.glLinkProgram(program_handle);
    gl.glValidateProgram(program_handle);

    final IntBuffer buffer = Buffers.newDirectIntBuffer(1);
    gl.glGetProgramiv(program_handle, GL3.GL_VALIDATE_STATUS, buffer);

    if (buffer.get(0) == 1) {
      return program_handle;
    } else {

      gl.glGetProgramiv(program_handle, GL3.GL_INFO_LOG_LENGTH, buffer);

      final ByteBuffer byteBuffer = Buffers.newDirectByteBuffer(buffer
              .get(0));
      gl.glGetProgramInfoLog(program_handle, byteBuffer.capacity(), buffer,
              byteBuffer);
