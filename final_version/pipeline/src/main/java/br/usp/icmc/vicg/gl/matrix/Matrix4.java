...
public class Matrix4 {
    ...
    public void lookAt(float x0, float y0, float z0,
        ...
        float[] v = cross(n, u);
        normalize(v); // matematicamente não precisaria disso
    ...
    // Transforma o volume do frustum/tronco de piramide num volume cúbico normalizado
    ...
    public float[] multiplyByVector(float[] vec) {
        // To perform Frustum Culling in CPU
        float[] r = new float[4];
        // Cache the matrix values (makes for huge speed increases!)
        float a00 = this.matrix[ 0], a01 = this.matrix[ 1], a02 = this.matrix[ 2], a03 = this.matrix[3];
        float a10 = this.matrix[ 4], a11 = this.matrix[ 5], a12 = this.matrix[ 6], a13 = this.matrix[7];
        ...
        float x=vec[0], y=vec[1], z=vec[2], h=(vec.length < 4) ? 1 : vec[3];
        r[0] = a00 * x + a01 * y + a02 * z + a03 * h;  
        r[1] = a10 * x + a11 * y + a12 * z + a13 * h;  
        r[2] = a20 * x + a21 * y + a22 * z + a23 * h;  
        r[3] = a30 * x + a31 * y + a32 * z + a33 * h;
        
        return r;
    }
    
//     public Point3D multiplyByVector(Point3D vec) {
//        // To perform Frustum Culling in CPU
...
//        float a30 = this.matrix[12], a31 = this.matrix[13], a32 = this.matrix[14], a33 = this.matrix[15];
//        
//        float x=vec.x, y=vec.y, z=vec.z, h=1;
//        r[0] = a00 * x + a01 * y + a02 * z + a03 * h;  
//        r[1] = a10 * x + a11 * y + a12 * z + a13 * h;  
//        r[2] = a20 * x + a21 * y + a22 * z + a23 * h;  
//        r[3] = a30 * x + a31 * y + a32 * z + a33 * h;
